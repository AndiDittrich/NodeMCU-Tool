...
NodeMcuConnector.prototype.checkConnection = function(cb){
    ...
    setTimeout(function() {
        ...
        this.device.executeCommand(_luaCommandBuilder.command.echo, function(err, echo, output){
            if (err){
                ...
                // validate command echo and command output - allow output to contain the end of boot messages just in case we didn't wait long enough
                if (output.match(/echo1337/) && echo == 'print("echo1337")') {
                    cb(null);
                } else {
    ...
    }.bind(this), 1000); // Wait 1000ms for boot (some devices auto reset on a new serial connection)
};
